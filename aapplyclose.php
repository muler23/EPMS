...
print("ይቅርታ የመመዝገቢያው ቀን አልቋል!!");
echo "<br>የመጨረሻው ቀን በ : " . $closedate;
?>
<br>
<script>
...
function CountBack(secs) {
  if (secs < 0) {
document.getElementById("cntdwn").innerHTML = "ተዘግቷል";
    return;
  }
...
-->   
  <p align="right"><font color="#387cc7">ጥያቄዎትን መላኪያ ጊዜው ተዘግቷል  !!!</font></p>                    
  <?php
			...
			<td> የተጠቃሚ ስም: </td><td><input type="text" name="un" required="1" class="textbox" onKeyPress="return ValidateAlpha(event)"/></td>
			  ...
			  <td> የይለፍ ቃል: </td><td><input type="password" name="pw" required="1" class="textbox" id="pw" />
			  				 						<script type="text/javascript">
			 ...
			 <td>የይለፍ ቃሉ የደገም: </td><td><input type="password" name="cpw" required="1" class="textbox" id="rpw"/></td>
			 </tr>
		<tr><td>ጾታ:</td><td><input type="text" name="gender" class="textbox" required="1" readonly value="<?php echo $row['gender'];?>"/></td>
		<td>የተጠቃሚ ፎቶ:</td><td><input type="file" name="photo" id="file" required="1" accept="image/*" onchange="loadFile(event)"/></td>
			 </tr>
			 <tr><td> አድሜ: </td>
			 ...
			 <td><input type="text" name="collage" class="textbox" required="1" readonly value="<?php echo $row['collage'];?>"></td></tr>
		      ...
		      <tr><td>የት.ት አመት: </td><td><input type="text" name="year" class="textbox" required="1" readonly value="<?php echo $row['year'];?>"></td></tr>
...
echo "<font color=red><center>ይቅርታ የዩኒቨርስቲው ተማሪ ስላልሆኑ አገልግሎቱን ማግኘት አይችሉም .</center></font>";
}
		...
		$sql=mysql_query("insert into requesttable values('$cid','$fname','$lname','$gender','$age','$collage','$dept','$year','$cgpa','$username','$pass','Candidate','$photopath','no','$date')");
				if(!$sql)
				die("<font color=red><b><center>ካንዴ በላይ መወዳደር አይቻልም</center></b></font>".mysql_error());
					else
						echo "<font color=green><b><center>በትክክል ተመዝግበዋል!</center></b></font>";
				...
				{
				if($psize>1000000)
				echo "ፎቶው ከ 2Kb በላይ መሆን የለበትም!";
				else
				echo "ፎቶው በjpeg ፎርም መቀመጥ አለበት";
				}
				}
				else
				echo "<font color=red><b><center>የኤክስቴንሽን ተማሪ ስለሆኑ ከውድድሩ መሳተፍ አይችሉም.</center></b></font>"; 
				}
				else
