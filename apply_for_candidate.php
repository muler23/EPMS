...
echo "<font color=red><center>You are not The member of University!<br>So You Couldn't apply for candidate.</center></font>";
		...
		else if(isset($_POST['register']))
				{
					...
					$ptype=$_FILES["photo"]["type"];
						...
						{
							 // $sql=mysql_query("SELECT * FROM student  JOIN  discipline ON student.sid=discipline.sid where student where sid='$cid' and  where discipline.disciplineType='none'");
				...
				$userexist=mysql_query($sql);
						while($row=mysql_fetch_assoc($userexist))
						{
							...
							if($gpa>="3.00" && $gender=="male" ||$gpa>="2.78" && $gender=="female")
							{
								if ($year=="2" || $year=="3" ) {
		...
		$sql=mysql_query("insert into requesttable values('$cid','$fname','$lname','$gender','$age','$collage','$dept','$year','$cgpa','$username','$pass','Candidate','unvote','$photopath','no','$date')");
				...
				echo "<font color=red><b><center>You Couldn't Apply For candidate Because  you are Extension Student.</center></b></font>"; 
					...
					echo "<font color=red><b><center>You Couldn't Apply For candidate Because  you are 
				$year 'st year  Student be 2nd or 3rd year student to be candidate.</center></b></font>"; }
				else
				echo "<font color=red><b><center>Your CGPA should be Greater than or equal to 3.00 for male and Greater than or equal to 2.75 for female  !<br>So You Couldn't apply for candidate.</center></b></font>";             }
			else
